...
if user_option == 'transactions':
    ...
    if user_choice == 'aggregated':
        ...
        Q = f"SELECT state_name,count,amount FROM top_tx_df WHERE year = '{user_year}' and quarter = '{user_quarter}' "#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        # fig = px.pie(df, values='count', names='name', title='Population of European continent',hover_data=['amount'],hover_name='state_name')
        ...
        Q=f"SELECT state_name,count,amount FROM top_tx_df WHERE year = '{user_year}' and quarter = '{user_quarter}' order by amount desc;"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q1}")
        # st.write(df)
        ...
        Q = f"SELECT state_name,name,count,amount FROM top_district_tx_df WHERE year = '{user_year}' and quarter = '{user_quarter}' and state_name in {tuple(states_list)[:]} order by amount desc;"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT state_name,name,count,amount FROM top_pincode_tx_df WHERE year = '{user_year}' and quarter = '{user_quarter}' and state_name in {tuple(states_list)[:]} order by amount desc;"
        df = pd.read_sql(Q,mydb)
        # st.write(df)
        fig = px.sunburst(df, path=['state_name', 'name'], values='amount',)
        st.plotly_chart(fig)
        ...
        Q=f"SELECT state_name,count,amount FROM map_tx_df WHERE year = '{user_year}' and quarter = '{user_quarter}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q1}")
        # st.write(df)
        fig = px.pie(df, values='count', names='state_name', title='Population of European continent',hover_data=['amount'])
    ...
    if user_choice == 'aggregated':
        Q=f"SELECT brand,count,percentage FROM aggregated_user_df WHERE year = '{user_year}' and quarter = '{user_quarter}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q1}")
        # st.write(df)
        ...
        Q = f"SELECT state_name,sum(count) as count FROM aggregated_state_user_df WHERE year = '{user_year}' and quarter = '{user_quarter}' group by state_name"#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        # fig = px.pie(df, values='count', names='name', title='Population of European continent',hover_data=['amount'],hover_name='state_name')
        ...
        Q = f"SELECT brand,count,percentage FROM aggregated_state_user_df WHERE year = '{user_year}' and quarter = '{user_quarter}' and state_name = '{user_state}' ;"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT state_name,brand,count,percentage FROM aggregated_state_user_df WHERE year = '{user_year}' and quarter = '{user_quarter}' ;"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT brand,count,percentage,year FROM aggregated_state_user_df WHERE brand = '{user_brand}' and quarter = '{user_quarter}' and state_name ='{user_state}'"#in {tuple(states_list)[:]} order by amount desc;"
        df = pd.read_sql(Q,mydb)
        # st.write(df)
        # fig = px.sunburst(df, path=['state_name', 'name'], values='amount',)
        # st.plotly_chart(fig)
        ...
        Q = f"SELECT state_name,registeredUsers FROM top_user_state WHERE year = '{user_year}' and quarter = '{user_quarter}' "#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT district_name,registeredUsers FROM top_user_district WHERE year = '{user_year}' and quarter = '{user_quarter}' "#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT pincode,registeredUsers FROM top_user_pincode WHERE year = '{user_year}' and quarter = '{user_quarter}' "#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
        ...
        Q = f"SELECT state_name,district_name,registeredUsers FROM top_user_district_by_state WHERE year = '{user_year}' and quarter = '{user_quarter}' order by registeredUsers desc"#and state_name = '{user_state}';"
        df = pd.read_sql(Q,mydb)
        # st.write(f"Query: {Q2}")
        # st.write(df)
